 ...
 */
public class  Auton_3T0B extends Command implements Pathable {
	/**
	 *Autonomous command; grabs the nearest tote and bin, turns to grab another tote, and moves to the autozone.
	 */
    public Auton_3T0B() {
	...
	public ArrayList<Instruction> getPath() {
    	...
    	else
    	{
    		...
    		path.add(new FieldLocation(Robot.startPositions[3].x, Robot.startPositions[3].y - .45, Robot.startPositions[3].heading)); //move a bit forward
    		path.add((Instruction) new ToggleIngestors()); //CLOSE
    		path.add((Instruction) new SpinRollers(false, false, 300));
    	
    		path.add(new Delay(0.5));
    		
    		//pick up the 2nd tote
    		path.add((Instruction) new ToggleIngestors()); //OPEN
        	...
        	//move to the third tote and grab
        	path.add(new FieldLocation(Robot.startPositions[1].x, Robot.startPositions[1].y - .65, Robot.startPositions[1].heading));
        	path.add((Instruction) new ToggleIngestors()); //CLOSE
        	path.add((Instruction) new SpinRollers(false, false, 300));
    		...
    		FieldLocation midPosition = new FieldLocation(startPosition.x + Global.distToMidPos + 0.25, startPosition.y - .65, 0);
            FieldLocation endPosition =  new FieldLocation(startPosition.x + Global.distToEndPos + 0.15, startPosition.y - .65, 0);
